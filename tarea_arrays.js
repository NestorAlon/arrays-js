...
function procesarPedido(pedido) {
    const clienteNombre = pedido.shift();
    pedido.unshift("bebida");
    pedido.push(clienteNombre);
    return pedido;
}
...
function buscaPalabras(words, word) {
    ...
    const largoBuscada = word.length;

    return words.filter(palabra => palabra.length > largoBuscada);
}
...
function findMinMaxPages(books) {
    ...
    for (let i = 1; i < books.length; i++) {
        if( books[i] < books[indiceMin] ) indiceMin = i;
        if( books[i] > books[indiceMax] ) indiceMax = i;
    }
